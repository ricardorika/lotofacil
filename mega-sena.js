// Função para buscar os últimos jogos da Mega-Sena
async function buscarUltimosJogos(quantidade = 100) {
    try {
        // Usando a API da Mega-Sena
        const response = await fetch(`https://loteriascaixa-api.herokuapp.com/api/mega-sena/latest/${quantidade}`);
        if (!response.ok) {
...
function salvarJogo(numeros, tipo) {
    const jogos = JSON.parse(localStorage.getItem('ultimosJogosMegaSena') || '[]');
    const novoJogo = {
    ...
    localStorage.setItem('ultimosJogosMegaSena', JSON.stringify(jogosLimitados));
...
function atualizarHistoricoJogos() {
    ...
    jogosFiltrados.forEach(jogo => {
        ...
        const numerosHtml = jogo.numeros.map(num => 
            `<span class="bg-green-600 text-white px-2 py-1 rounded-full text-sm mr-1">${num}</span>`
        ).join('');
