// Função para buscar os últimos jogos da Lotomania
async function buscarUltimosJogos(quantidade = 100) {
    try {
        // Usando a API da Lotomania
        const response = await fetch(`https://loteriascaixa-api.herokuapp.com/api/lotomania/latest/${quantidade}`);
        if (!response.ok) {
...
function gerarDadosExemplo(quantidade) {
    ...
    for (let i = 0; i < quantidade; i++) {
        ...
        jogos.push({
            concurso: i + 1,
            dezenas: numeros.slice(0, 50).sort((a, b) => a - b)
        });
    }
    return jogos;
}
...
function gerarNumerosBaseadosEmEstatisticas(estatisticas) {
    const numeros = [];
    const numerosDisponiveis = Array.from({length: 100}, (_, i) => i);
...
function salvarJogo(numeros, tipo) {
    const jogos = JSON.parse(localStorage.getItem('ultimosJogosLotomania') || '[]');
    const novoJogo = {
    ...
    localStorage.setItem('ultimosJogosLotomania', JSON.stringify(jogosLimitados));
...
function atualizarHistoricoJogos() {
    ...
    jogosFiltrados.forEach(jogo => {
        ...
        const numerosHtml = jogo.numeros.map(num => 
            `<span class="bg-red-600 text-white px-2 py-1 rounded-full text-sm mr-1">${num}</span>`
        ).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Criar os 100 números (0 a 99)
    for (let i = 0; i <= 99; i++) {
        const numero = document.createElement('button');
        numero.className = 'w-12 h-12 rounded-full border-2 border-red-600 text-lg font-semibold hover:bg-red-100 transition-colors';
    ...
    function gerarJogoAleatorio() {
        ...
        numerosSelecionados = new Set(numeros.slice(0, 50));
        atualizarNumerosSelecionados();
    ...
    function confirmarJogoManual() {
        if (numerosSelecionados.size === 50) {
            salvarJogo(numerosSelecionados, 'manual');
            modoManual = false;
            btnConfirmarJogo.classList.add('hidden');
        } else {
            alert('Selecione exatamente 50 números antes de confirmar!');
        }
    }
