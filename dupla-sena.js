// Função para buscar os últimos jogos da Dupla Sena
async function buscarUltimosJogos(quantidade = 100) {
    try {
        // Usando a API da Dupla Sena
        const response = await fetch(`https://loteriascaixa-api.herokuapp.com/api/dupla-sena/latest/${quantidade}`);
        if (!response.ok) {
...
function gerarDadosExemplo(quantidade) {
    ...
    for (let i = 0; i < quantidade; i++) {
        ...
        jogos.push({
            concurso: i + 1,
            dezenas: numeros.slice(0, 6).sort((a, b) => a - b)
        });
    }
    return jogos;
}
...
function gerarNumerosBaseadosEmEstatisticas(estatisticas) {
    const numeros = [];
    const numerosDisponiveis = Array.from({length: 50}, (_, i) => i + 1);
...
function salvarJogo(numeros, tipo) {
    const jogos = JSON.parse(localStorage.getItem('ultimosJogosDuplaSena') || '[]');
    const novoJogo = {
    ...
    localStorage.setItem('ultimosJogosDuplaSena', JSON.stringify(jogosLimitados));
...
function atualizarHistoricoJogos() {
    ...
    jogosFiltrados.forEach(jogo => {
        ...
        const numerosHtml = jogo.numeros.map(num => 
            `<span class="bg-yellow-600 text-white px-2 py-1 rounded-full text-sm mr-1">${num}</span>`
        ).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 1; i <= 50; i++) {
        ...
        numero.className = 'w-12 h-12 rounded-full border-2 border-yellow-600 text-lg font-semibold hover:bg-yellow-100 transition-colors';
    ...
    function toggleNumero(num, elemento) {
        if (numerosSelecionados.has(num)) {
            ...
            elemento.classList.remove('bg-yellow-600', 'text-white');
            elemento.classList.add('border-yellow-600');
            ...
            elemento.classList.add('bg-yellow-600', 'text-white');
            elemento.classList.remove('border-yellow-600');
            atualizarNumerosSelecionados();
            modoManual = true;
            btnConfirmarJogo.classList.remove('hidden');
        } else {
    ...
    function gerarJogoAleatorio() {
        ...
        numerosSelecionados = new Set(numeros.slice(0, 6));
        atualizarNumerosSelecionados();
    ...
    function confirmarJogoManual() {
        if (numerosSelecionados.size === 6) {
            ...
            alert('Selecione exatamente 6 números antes de confirmar!');
        }
    }
